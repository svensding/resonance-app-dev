            ...
            <button
                ...
                className={`p-2 rounded-full transition-colors duration-150 relative
                            ${!isTtsAvailable 
                                ...
                                : `text-slate-400 ${disabled ? 'cursor-not-allowed opacity-50' : 'hover:bg-slate-600 hover:text-white'}`
                            }`}
                aria-label={!isTtsAvailable ? "Voice Settings (Service Unavailable)" : "Open Voice and Audio Settings"}
                title={!isTtsAvailable ? "Voice Service Currently Unavailable" : "Voice & Audio Settings"}
            >
                {!isTtsAvailable ? (
                    // Speaker X Mark Icon for Unavailable State
                ...
                {!isTtsAvailable && (
                    <span className="absolute top-1 right-1 h-2 w-2 rounded-full bg-amber-500 shadow-sm animate-pulse"></span>
                )}
            </button>
            {showDevLogButton && (
                <button
